...
class BaselineSearchAgent(CompetitionAgent):
  """
    This is a baseline search based agent to see if you can do better.
    A search based agaent uses a simple A* search to find an efficient
    path to the next food point (whilst trying to avoid the ghosts)
  """
  ...
  def getAction(self, gameState):
    ...
    the time limit (otherwise a random legal action will be chosen for you).
    """
    # Define a (custom?) problem for the current state.
    p = AnyFoodSearchProblem(gameState)
    # Solve it using A* or some other search algorithm.
    path = aStarSearch(p)
    if path:
        # Return the first action of the found path.
        return path[0]

    # Randomly choose a legal move
    legalMoves = gameState.getLegalActions()
    chosenIndex = random.choice(range(len(legalMoves))) # Pick randomly among the best
    return legalMoves[chosenIndex]
...
MyPacmanAgent=BaselineSearchAgent
