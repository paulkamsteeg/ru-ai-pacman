...
GHOST_SIZE = 0.65
SCARED_COLOR = gU.formatColor(1, 1, 1)

GHOST_VEC_COLORS = list(map(gU.colorToVector, GHOST_COLORS))

PACMAN_COLOR = gU.formatColor(255.0 / 255.0, 255.0 / 255.0, 61.0 / 255)
PACMAN_SCALE = 0.5
# pacman_speed = 0.25

# Food
FOOD_COLOR = gU.formatColor(1, 1, 1)
FOOD_SIZE = 0.1

# Laser
LASER_COLOR = gU.formatColor(1, 0, 0)
LASER_SIZE = 0.02

# Capsule graphics
CAPSULE_COLOR = gU.formatColor(1, 1, 1)
CAPSULE_SIZE = 0.25
...
class InfoPane:
    ...
    def drawPane(self):
        self.scoreText = gU.text(self.toScreen(0, 0), self.textColor,
                                 "SCORE:    0", "Times",
                                 self.fontSize, "bold")
    ...
    def initializeGhostDistances(self, distances):
        ...
        for i, d in enumerate(distances):
            t = gU.text(self.toScreen(self.width / 2 + self.width / 8 * i, 0),
...
class PacmanGraphics:
    ...
    def moveEyes(self, pos, dir, eyes):
        ...
        if dir == 'West':
            dx = -0.2
        gU.moveCircle(eyes[0],
                      (screen_x + self.gridSize * GHOST_SIZE * (-0.3 + dx / 1.5),
                       screen_y - self.gridSize * GHOST_SIZE * (0.3 - dy / 1.5)),
                      self.gridSize * GHOST_SIZE * 0.2)
        gU.moveCircle(eyes[1], (screen_x + self.gridSize * GHOST_SIZE * (0.3 + dx / 1.5),
                                screen_y - self.gridSize * GHOST_SIZE * (0.3 - dy / 1.5)),
                      self.gridSize * GHOST_SIZE * 0.2)
        gU.moveCircle(eyes[2], (screen_x + self.gridSize * GHOST_SIZE * (-0.3 + dx),
                                screen_y - self.gridSize * GHOST_SIZE * (0.3 - dy)),
                      self.gridSize * GHOST_SIZE * 0.08)
        gU.moveCircle(eyes[3], (screen_x + self.gridSize * GHOST_SIZE * (0.3 + dx),
                                screen_y - self.gridSize * GHOST_SIZE * (0.3 - dy)),
                      self.gridSize * GHOST_SIZE * 0.08)
...
class FirstPersonPacmanGraphics(PacmanGraphics):
    ...
    def initialize(self, state, isBlue=False):
        ...
        """
        The following two variables are local and never used, dont know why
        they are here
        """
        # walls = state.layout.walls
        # dist = []
    ...
    def getPosition(self, ghostState):
        if not self.showGhosts and not ghostState.isPacman\
...
def saveFrame():
    ...
    FRAME_NUMBER += 1
    gU.writePostscript(name)  # writes the current canvas
