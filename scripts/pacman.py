...
class GameState:
    ...
    the actual state of the game and
    can be used by agents to reason about the game.

    Much of the information in a GameState is stored in
    a GameStateData object.  We strongly suggest that you
    access that data via the accessor methods below rather
    ...
    def decrementTimer(ghostState):
        ...
        if timer == 1:
            ghostState.configuration.pos =\
                nearestPoint(ghostState.configuration.pos)
...
def readCommand(argv):
    ...
    parser.add_option('-l', '--layout', dest='layout',
                      help=default('the LAYOUT_FILE from which \
                          to load the map layout'),
                      metavar='LAYOUT_FILE', default='mediumClassic')
    parser.add_option('-p', '--pacman', dest='pacman',
                      help=default('the agent TYPE in the \
                              pacmanAgents module to use'),
                      metavar='TYPE', default='KeyboardAgent')
    parser.add_option('-t', '--textGraphics', action='store_true',
                      dest='textGraphics',
                      help='Display output as text only', default=False)
    parser.add_option('-q', '--quietTextGraphics', action='store_true',
                      dest='quietGraphics', help='Generate minimal \
                      output and no graphics', default=False)
    parser.add_option('-g', '--ghosts', dest='ghost',
                      help=default('the ghost agent TYPE in \
                      the ghostAgents module to use'),
    ...
    parser.add_option('-f', '--fixRandomSeed', action='store_true',
                      dest='fixRandomSeed', help='Fixes the random seed \
                      to always play the same game', default=False)
    parser.add_option('-r', '--recordActions', action='store_true',
                      dest='record', help='Writes game histories to a \
    ...
    parser.add_option('--timeout', dest='timeout', type='int',
                      ...
                      spend computing in a single game'), default=30)
    ...
    noKeyboard = options.gameToReplay is None and (options.textGraphics or
    ...
    if options.quietGraphics:
        ...
        args['display'] = graphicsDisplay.\
            PacmanGraphics(options.zoom, frameTime=options.frameTime)
    ...
    """
    Special case: recorded games don't use the
    runGames method or args structure.
    """
        ...
        for modulename in moduleNames:
            ...
            if pacman in dir(module):
                if nographics and modulename == 'keyboardAgents.py':
                    raise Exception('Using the keyboard requires \
...
if __name__ == '__main__':
    ...
    """
    if len(sys.argv) == 1:
        sys.argv.extend([x for x in
                         re.split(r' *', input("Enter any command\
line arguments?")) if x != ''])
