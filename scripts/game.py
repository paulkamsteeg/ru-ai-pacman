...
class Configuration:
    ...
    The convention for positions, like a graph,
    is that (0,0) is the lower left corner, x increases
...
class Grid:
    ...
    def __init__(self, width, height,
                 initialValue=False, bitRepresentation=None):
    ...
    def reconstituteGrid(bitRep):
        if not ut.isInstance(bitRep, (1, 2)):
            return bitRep
        ...
        for isPacman, pos in layout.agentPositions:
            if not isPacman:
                if numGhosts == numGhostAgents:
                    ...
                    numGhosts += 1
            self.agentStates.append(AgentState(Configuration
                                               (pos, Directions.STOP), isPacman))
...
class Game:
    ...
    def unmute(self):
        ...
        sys.stdout = ut.OLD_STDOUT
        sys.stderr = ut.OLD_STDERR
    ...
    def run(self):
        ...
        while not self.gameOver:
            ...
            if self.catchExceptions:
                try:
                    timed_func = ut.TimeoutFunction(agent.getAction,
                                                    int(self.rules.
                                                        getMoveTimeout
                                                        (agentIndex)) -
                                                    int(move_time))
