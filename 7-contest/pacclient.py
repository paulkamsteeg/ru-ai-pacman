  ...
  def run(self):
    while not self.gameOver:
      ...
      if not self.gotGameStateEvent.isSet():
        print('Lost connection with server', file=sys.stderr)
...
class PacClient(asyncore.dispatcher):
  ...
  def __init__(self, server, port, agentFactoryBuilder, display, user, password, gamename):
    ...
    print("Connecting to server(%s:%d)..." % (server, port), file=sys.stderr)
  ...
  def parse_line(self,line):
    if int(line.split(":")[0]) in list(self.responses.keys()):
  ...
  def error(self,code,line):
    print("General error: ", file=sys.stderr)
    print(line, file=sys.stderr)

  def notify(self,code,line):
    print("The server requests our attention:", file=sys.stderr)
    print(line, file=sys.stderr)
  ...
  def handle_pickle(self, pickle):
    ...
    stateNumber, gameState = pickle.loads(pickle)
